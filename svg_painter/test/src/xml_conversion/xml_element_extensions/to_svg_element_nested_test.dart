...
void main() {
  test('nested svg should be SvgRoot? or SvgSvg?', () {
    final document = XmlDocument.parse(
      '<svg xmlns="http://www.w3.org/2000/svg"><svg id="nested" /></svg>',
    );
    final XmlElement rootElement = document.rootElement;
    final Result<SvgElement> result = rootElement.toSvgElement();
    final SvgRoot root = result.fold(
      (Failure<SvgElement> f) => throw Exception(f.message),
      (SvgElement v) => v as SvgRoot,
    );
