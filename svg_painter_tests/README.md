...
- Most developers use **ARM-based Macs** (M1/M2/M3/M4)
...
```
painter_name_fixed.png          # Default/CI golden (generated by CI workflow)
painter_name_fixed.mac_os.png   # macOS golden (used locally on Mac)
painter_name_viewBox.png        # Default/CI golden
...
- On CI: writes to `name.png` (the default golden)
...
The CI environment (currently x86 Linux) is the source of truth for default goldens:

```bash
# CI runs these commands:
flutter test                  # Run tests (uses default .png goldens)
flutter test --update-goldens # Update default/CI goldens
```

### Via Docker on macOS (Not Recommended)

You can attempt to generate CI-compatible goldens locally using Docker:
...
|-----------|-------------|
| `name_fixed.png` | Fixed 200x200 test, default/CI golden |
| `name_fixed.mac_os.png` | Fixed 200x200 test, macOS-specific golden |
| `name_viewBox.png` | ViewBox-sized test, default/CI golden |
