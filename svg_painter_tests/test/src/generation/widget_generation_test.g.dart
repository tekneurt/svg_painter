...
class AccessibleWidgetTestWidget extends StatelessWidget {
  const AccessibleWidgetTestWidget({
    super.key,
    this.width,
    this.height,
    this.fit = BoxFit.contain,
  ...
  Widget build(BuildContext context) {
    return Semantics(
      ...
      hint: 'A circle representing an accessible widget',
      child: CustomPaint(
        size: Size(width ?? 100.0, height ?? 100.0),
        painter: _$AccessibleWidgetTest(fit: fit),
      ),
    );
  }
}

class _$AccessibleWidgetTest extends CustomPainter {
  const _$AccessibleWidgetTest({this.fit = BoxFit.contain});
  ...
  @override
  bool shouldRepaint(covariant _$AccessibleWidgetTest oldDelegate) {
    if (fit == oldDelegate.fit) {
      return false;
    } else {
      return true;
